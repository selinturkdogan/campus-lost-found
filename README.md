...
- `main.dart` — bootstrap and route table
- `firebase_options.dart` — generated by FlutterFire CLI
- **`models/`**
  - `listing_model.dart` — Hive-typed listing object
- **`providers/`**
  - `auth_provider.dart`, `listings_provider.dart`, `post_form_provider.dart`
- **`services/`**
  - `comment_service.dart` — comment CRUD and @-mention search
...
- **`screens/`**
  ...
  - `home/` — home shell + feed tab
  - `detail/` — listing detail
  - `post/` — listing create/edit form
  - `my_posts/`, `messages/`, `chat/`, `notifications/`
  - `profile/` — profile + edit profile
  - `admin/` — admin panel + domain settings
  - `splash_screen.dart`
- **`widgets/`**
  - `listing_card.dart` — Instagram-style listing card
  - `comments_section.dart` — composer + mention picker
  - `user_avatar.dart`
- **`utils/`**
  - `app_routes.dart`, `app_theme.dart`, `image_utils.dart`

**`functions/`** — Cloud Functions source (Node.js)
...
- `logo/` — Constellation Search logo set (`png/`, `svg/`, `android/`, `ios/`)
